...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-white text-[#06131d]">
      <header>
        <div className="mx-auto max-w-[980px] px-4 py-12 sm:px-6 lg:px-0 lg:py-16">
          <BackLink task={task} />
          <div className="mt-12 flex flex-wrap items-center gap-3 text-[12px] tracking-[.16em] text-[#3B7597]">
            <span>{categoryOf(post, 'Media Distribution')}</span>
            <span className="h-px w-10 bg-[#6FD1D7]" />
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mt-5 max-w-5xl text-4xl font-black leading-[1.08] tracking-[-.035em] sm:text-5xl lg:text-6xl">{post.title}</h1>
          {summaryText(post) ? <p className="mt-6 max-w-3xl text-lg font-semibold leading-8 text-[#405c6b]">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[980px] border-y border-[#edf1f3] bg-white px-4 py-6 sm:px-6 lg:px-0">
          <img src={images[0]} alt="" className="max-h-[560px] w-full object-cover" />
          <figcaption className="mt-3 text-xs italic text-[#6e8793]">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1160px] gap-14 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-0 lg:py-16">
        <article className="min-w-0 border-t border-[#edf1f3] pt-8">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="border-t border-[#6FD1D7] pt-5">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="border-b border-[#edf1f3] bg-white pb-5">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[#6e8793]">About this post</p>
          <div className="mt-4 grid gap-3 text-sm font-bold text-[#405c6b]">
      ...
      {related.length ? (
        <div className="border-b border-[#edf1f3] bg-white pb-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-3xl font-black tracking-[-0.04em]">Popular Posts</h2>
            <Link href={taskConfig?.route || '/'} className="text-xs font-black uppercase tracking-[0.16em] text-[#6e8793]">View all</Link>
          </div>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#edf1f3] py-3 transition hover:text-[#3B7597]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center bg-[#093C5D] text-white"><FileText className="h-6 w-6" /></div>}
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-black leading-tight tracking-[-0.03em]">{post.title}</h3>
        <p className="mt-2 line-clamp-2 text-xs leading-5 text-[#6e8793]">{summaryText(post)}</p>
      </div>
    </Link>
  )
}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 border-t border-[#edf1f3] bg-white pt-8">
      <div className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="border-t border-[#edf1f3] py-4">
            <p className="text-sm font-black">{comment.name}</p>
            <p className="mt-2 text-sm leading-6 text-[#405c6b]">{comment.comment}</p>
          </div>
        ))}
        {!comments.length ? <p className="text-sm text-[#6e8793]">No comments yet for {slug}.</p> : null}
      </div>
    </section>
  )
}
